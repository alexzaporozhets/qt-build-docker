...
rmdir %CWD%\build-qthybrid-app-%SRCVER% /s /q
mkdir %CWD%\build-qthybrid-app-%SRCVER%
...
set DSTDIR=%CWD%\install-qthybrid-app-%SRCVER%\%TARGET%
rmdir %DSTDIR% /s /q
mkdir %DSTDIR%
...
%CWD%\zip -9 -r mystaff-client-%SRCVER%-windows-deployed.zip %TARGET%
if "%errorlevel%" NEQ "0" (
    cd %CWD%
    exit 8
) 
...
echo "Done: install-qthybrid-app-%SRCVER%\mystaff-client-%SRCVER%-windows.zip"
echo "      install-qthybrid-app-%SRCVER%\mystaff-client-pdb-%SRCVER%-windows.zip"
exit 0
